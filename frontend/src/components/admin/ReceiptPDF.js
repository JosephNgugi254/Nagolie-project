...
export const generateClientStatement = async (client, allTransactions) => {
  // Filter transactions for this client (assume transactions have loan_id matching client.loan_id)
  const clientTransactions = allTransactions.filter(t => t.loan_id === client.loan_id);
  ...
  const INTEREST_RATE = 30; // Hardcoded; make dynamic if needed
  ...
  loanDetails.forEach(({ label, value }) => {
    ...
    doc.text(label, 20, yPos);
    doc.setFont('helvetica', 'normal');
    doc.text(String(value), 70, yPos); // Indent more for longer labels
    yPos += 7;
  });
  ...
  // Save PDF
  doc.save(`Statement_${client.idNumber || client.name}_Receipt.pdf`);
};